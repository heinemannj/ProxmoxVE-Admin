...
function x509_list() {
  ...
  whiptail_textbox "Certificates by step" "$CERT_PATH/x509/x509Certs.txt"
...
function x509_view(){
  ...
  VALID_CERTS=$(grep -c "valid" "$CERT_PATH/x509/x509Certs.txt")
  EXPIRED_CERTS=$(( TOTAL_CERTS - VALID_CERTS ))
  echo -e "\n" >> "$CERT_PATH/x509/x509Certs.txt"
...
function x509_certs_menu() {
  ...
  local CHOICE
  x509_view
  [[ ${#CERT_LIST[@]} -gt 0 ]] && CHOICE=$(whiptail_checklist "Certificates by step" "\nSelect Certificate(s) to ${CERT_ACTION}:" "CERT_LIST")
  if [[ -z $CHOICE ]]; then
    x509_maintenance_menu
  else
